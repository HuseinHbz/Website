...
## 🔴 وضعیت واقعی تولید — به‌روزرسانی فاز ۲۶.۳۰ (۱۴۰۵/۰۵/۱۲)

نمرات پایین این سند **کیفیت کد و آمادگی سیستم** را می‌سنجند و معتبرند. ولی
«آماده برای تولید» با «در حال استفاده در تولید» یکی نیست، و این تمایز تا اینجا
در این سند دیده نمی‌شد:

| واقعیت | وضعیت |
|---|---|
| سرور تولید (`habibazar.ir`) | 🔴 **هفت فاز عقب** — کارهای ۲۶.۲۷ تا ۲۶.۳۳ روی آن نیست |
| علت انسداد | رمز PostgreSQL بین دیتابیس و `.env.local` هم‌گام نیست |
| رفع | `deploy/fix-db-password.sh` آماده است — اجرا با maintainer |
| کلیدهای واقعی (مودیان/زرین‌پال/پیامک) | 🔴 هیچ‌کدام تنظیم و تست نشده |
| چرخهٔ مالی واقعی تأییدشده توسط حسابدار | 🔴 انجام نشده |
| مشتری واقعی در پورتال | 🔴 انجام نشده |
| تست بازیابی روی بکاپ تولید | 🔴 انجام نشده |

**بنابراین: سیستم _آمادهٔ_ تولید است، ولی هنوز _در_ تولید نیست.** تا وقتی
پایلوت واقعی اجرا نشده، این سند یک ارزیابی مهندسی است نه گواهی عملیاتی.

مسیر کامل: `docs/PILOT_RUNBOOK_FA.md` · وضعیت هر بند: `docs/governance/phase26.30-pilot-report.md`

درسی که فاز ۲۶.۳۰ همین حالا اثبات کرد: اولین اجرای اسکریپت سلامت‌سنجی، سه
صفحهٔ عمومی را پیدا کرد که با قطع لحظه‌ای دیتابیس ۵۰۰ می‌دادند، و یک باگ
مسدودکننده در کانبان که **دو فاز** تشخیص اشتباه خورده بود. هیچ‌کدام را ۸۷۷
تست و ۱۲ ممیزی سبز نگرفته بودند.
...
| Health monitor script | `deploy/health-check.sh` |
