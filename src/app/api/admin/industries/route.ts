...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = await ensureUniqueSlug(body as Record<string, unknown>, 'industries', 'industry')
    const db = getDb()
    const result = (await runOnce(user.id, 'industries', body, async () => db.insert(industries).values(payload as never).returning()))[0]
