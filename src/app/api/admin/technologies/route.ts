...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = await ensureUniqueSlug(body as Record<string, unknown>, 'technologies', 'tech')
    const db = getDb()
    const result = (await runOnce(user.id, 'technologies', body, async () => db.insert(technologies).values(payload as never).returning()))[0]
