...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // process → GL posting requires finance/admin authority.
    if (!['super_admin','administrator'].includes(auth.user.role)) return NextResponse.json({ error: 'Only finance admins may process payments' }, { status: 403 })
