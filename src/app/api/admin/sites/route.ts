...
export async function POST(req: NextRequest) {
  try {
    ...
    const db = getDb()
    const result = (await runOnce(user.id, 'sites', body, async () => db.insert(sites).values({ ...await ensureUniqueSlug(body as Record<string, unknown>, 'sites', 'site'), id: body.id || randomUUID(), createdBy: user.id } as never).returning()))[0]
