...
export async function GET(req: NextRequest) {
  ...
  { const deny = await checkTreePermission(user, 'brand.credentials', 'read'); if (deny) return deny }
  const db = getDb()
...
export async function POST(req: NextRequest) {
  ...
  { const deny = await checkTreePermission(user, 'brand.credentials', 'write'); if (deny) return deny }
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
