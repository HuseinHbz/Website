...
export async function GET() {
  try {
    const db = getDb()
    return NextResponse.json(await db.select().from(services).orderBy(asc(services.sortOrder)))
  } catch (e: unknown) {
    return apiError(e)
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    { const deny = await checkTreePermission(user, 'brand.services', 'write'); if (deny) return deny }
    const body = await guardJson(req)
    const { id: _id, updatedAt: _u, ...data } = body
    const db = getDb()
    const result = await db.insert(services).values({ ...ensureSlug(data as Record<string, unknown>, "service"), updatedBy: user?.id } as never).returning()
