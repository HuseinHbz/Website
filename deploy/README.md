...
> 🔴🔴🔴 **قانون طلایی سرور**: `/var/www/habibazar` هرگز با دستور دستی
> (`git pull`/`fetch`/`checkout`/`npm ci`/`npm run build`، چه با `sudo` چه بدون
> آن، چه به‌عنوان `root` چه هر کاربر دیگر) دست‌کاری نمی‌شود — **فقط**
> `sudo bash deploy/update.sh`. برای دیباگ مستقیم روی این مسیر (خواندن فایل،
> بررسی state):‌ با کاربر اپ وارد شوید — `sudo -u hbz -i` — نه با `root`. هر بار
> این قانون زیر پا گذاشته شود، دفعهٔ بعدِ `update.sh` با «Permission denied»
> شکست می‌خورد (سه بار در عمل رخ داده — رجوع کنید به
...
### `node_modules` هم — و هر مسیر نوشتنی دیگر (DEPLOY-FIX-2)

همان خانوادهٔ مشکل (مالکیت درفت‌کرده → `Permission denied` زیر `hbz`) اصولاً
می‌تواند روی هر مسیری که build/runtime در آن می‌نویسد رخ دهد، نه فقط سورس یا
`.next`. `update.sh` حالا:
- درست پیش از این‌که واقعاً `npm ci` اجرا شود (نه در مسیر «تغییر نکرده»، تا آن
  مسیر سریع بماند)، `node_modules` را هم ترمیم مالکیت می‌کند.
- پیش از هر ترمیمی، **می‌شمارد** چند مسیر مالکیت اشتباه دارند و در
  `/var/log/habibazar-ownership-drift.log` ثبت می‌کند (جدا از لاگِ هر اجرا) —
  تا الگوی تکرار (چند بار، کِی، چقدر) قابل ردیابی بماند، نه فقط بی‌صدا ترمیم شود.

### Divergent branches بعد از یک rollback (DEPLOY-FIX-2 بند ۴)

اگر بین دو اجرای `update.sh`، **default branch مخزن در گیت‌هاب عوض شود** (مثلاً
از `V3-Codex` به `claude/bold-lamport-a1d6tg`) و درست همان اجرا با شکست build
مواجه شود، rollback قدیم سعی می‌کرد کامیت قبلی را روی برنچِ **جدید** (با همان
نام) `reset --hard` کند — ولی آن کامیت قبلی متعلق به تاریخچهٔ برنچ **قبلی** بود،
پس git آن را «diverged» می‌دید (این دقیقاً چیزی است که در لاگ واقعی دیده شد: «3
and 2 different commits»؛ بی‌ضرر بود چون اجرای بعدی با `reset --hard
origin/…` دوباره درستش می‌کرد، ولی گمراه‌کننده بود). این یک عادت انسانی یا
کامیت دستی روی سرور **نبود** — یک نقص منطقی در rollback خودِ اسکریپت بود، حالا
رفع شده: اگر برنچ قبلی با برنچ فعلی فرق داشته باشد، rollback به‌جای ریست‌کردن
برنچ فعلی، به خودِ برنچ قبلی برمی‌گردد.
