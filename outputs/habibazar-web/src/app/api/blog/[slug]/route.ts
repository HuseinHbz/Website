...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    `, [slug]))[0] as Record<string, unknown> | undefined

    if (!row) return NextResponse.json({ error: 'Not found' }, { status: 404 })

    const navSelect = 'SELECT slug, title_en, title_fa FROM blog_posts WHERE status = $1 AND id '
    const prev = (await pgQuery(navSelect + '< $2 ORDER BY id DESC LIMIT 1', ['published', row.id as number]))[0] as { slug: string; title_en: string; title_fa: string } | undefined
    const next = (await pgQuery(navSelect + '> $2 ORDER BY id ASC LIMIT 1', ['published', row.id as number]))[0] as { slug: string; title_en: string; title_fa: string } | undefined
