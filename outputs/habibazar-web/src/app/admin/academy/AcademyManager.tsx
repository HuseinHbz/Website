...
export function AcademyManager() {
  ...
  return (
    <div>
      <ToastContainer />
      <PageHeader title="Academy Platform" subtitle="Courses, learning paths, certifications and labs"
        action={<Btn onClick={() => setEditing({ level: 'intermediate', type: 'course', status: 'draft', isFree: true, price: 0, lessonsCount: 0, enrollmentsCount: 0, rating: 0 })}>+ New Course</Btn>} />

      <div className="grid grid-cols-3 gap-4 mb-6">
        {[{ label: 'Total Courses', value: stats.total, icon: '🎓' }, { label: 'Published', value: stats.published, icon: '✅' }, { label: 'Total Enrollments', value: stats.enrolled, icon: '👥' }].map(s => (
          <div key={s.label} className="rounded-xl p-4 flex items-center gap-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div><div className="text-2xl font-black text-white">{s.value}</div><div className="text-xs text-slate-500">{s.label}</div></div>
          </div>
        ))}
      </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#0e0e1a] border border-slate-800 rounded-2xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-bold text-white mb-4">{editing.id ? 'Edit Course' : 'New Course'}</h3>
            <div className="grid grid-cols-2 gap-4">
              ...
              <Select label="Level" value={editing.level || 'intermediate'} onChange={v => setEditing(e => ({ ...e, level: v }))} options={LEVELS.map(l => ({ value: l, label: l }))} />
              <Select label="Type" value={editing.type || 'course'} onChange={v => setEditing(e => ({ ...e, type: v }))} options={TYPES.map(t => ({ value: t, label: t }))} />
              <Select label="Status" value={editing.status || 'draft'} onChange={v => setEditing(e => ({ ...e, status: v }))} options={STATUSES.map(s => ({ value: s, label: s }))} />
              <Input label="Duration (hours)" type="number" value={String(editing.durationHours || 0)} onChange={v => setEditing(e => ({ ...e, durationHours: parseInt(v) || 0 }))} />
      ...
      <Card>
        {loading ? <div className="text-center py-8 text-slate-500">Loading…</div> : courses.length === 0 ? (
          <div className="text-center py-12"><div className="text-4xl mb-3">🎓</div><div className="text-white font-medium mb-1">No courses yet</div><div className="text-slate-500 text-sm">Create your first course to launch the academy</div></div>
        ) : (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-slate-800 text-left">{['Course', 'Level', 'Type', 'Status', 'Enrolled', 'Actions'].map(h => <th key={h} className="px-4 py-3 text-slate-500 font-medium">{h}</th>)}</tr></thead>
            <tbody>
              {courses.map(c => (
                <tr key={c.id} className="border-b border-slate-800/50 hover:bg-white/[0.02]">
                  <td className="px-4 py-3"><div className="font-medium text-white">{c.titleEn}</div><div className="text-xs text-slate-500">{c.durationHours}h · {c.lessonsCount} lessons</div></td>
                  ...
                  <td className="px-4 py-3 text-slate-400">{c.enrollmentsCount}</td>
                  <td className="px-4 py-3"><Btn size="sm" variant="ghost" onClick={() => setEditing(c)}>Edit</Btn></td>
                </tr>
              ))}
            </tbody>
          </table>
