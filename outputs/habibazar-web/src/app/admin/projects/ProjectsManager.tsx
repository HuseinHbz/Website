    ...
    <>
      ...
      <Card>
        <DataTable
          ...
          columns={[
            { key: 'nameEn', labelEn: 'Name', labelFa: t('name'), value: p => isFa ? p.nameFa : p.nameEn, render: p => <div className="flex items-center gap-2"><ColorDot color={p.color} /><div><div className="font-medium text-white">{isFa ? p.nameFa : p.nameEn}</div><div className="text-xs text-text-tertiary">{isFa ? p.nameEn : p.nameFa} · {p.year}</div><div className="text-3xs text-text-disabled">{p.slug}</div></div></div> },
