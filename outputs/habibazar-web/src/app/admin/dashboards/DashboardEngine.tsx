...
import { Card, Btn, useToast } from '@/components/admin/ui'
...
type Size = 'sm' | 'md' | 'lg'
interface LayoutEntry { id: string; size: Size }
...
export function DashboardEngine({ workspace }: { workspace: string }) {
  ...
  const loadData = useCallback(async (ids: string[]) => {
    ...
    try {
      const r = await fetch(`/api/admin/dashboards/data?ids=${ids.join(',')}`)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const d = await r.json()
      setLayout(d.layout ?? []); setAvailable(d.available ?? [])
      await loadData((d.layout ?? []).map((e: LayoutEntry) => e.id))
  ...
  async function save() {
    try {
      const r = await fetch('/api/admin/dashboards', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace, layout }) })
      if (!r.ok) throw new Error()
      toast(t('dash_saved'), 'success'); setDirty(false)
  ...
  async function reset() {
    ...
    if (r.ok) { setLayout(d.layout ?? []); setDirty(false); loadData((d.layout ?? []).map((e: LayoutEntry) => e.id)); toast(t('dash_resetDone'), 'success') }
  }
    ...
    <div className="space-y-5" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold text-text-primary flex items-center gap-2"><span>{ws?.icon}</span>{isRTL ? ws?.nameFa : ws?.nameEn} — {t('dash_title')}</h1>
        ...
        {edit && dirty && <Btn onClick={save}>{t('dash_save')}</Btn>}
