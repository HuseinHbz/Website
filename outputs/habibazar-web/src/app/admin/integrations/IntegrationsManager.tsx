...
export function IntegrationsManager() {
  ...
  return (
    <div>
      <ToastContainer />
      <PageHeader title="Enterprise Integrations" subtitle={`${integrations.filter(i => i.enabled).length} active · ${integrations.length} total`} />

      {loading ? <div className="text-center py-16 text-slate-500">Loading…</div> : (
        <div className="space-y-8">
          {Object.entries(grouped).map(([cat, items]) => (
            <div key={cat}>
              <h3 className="text-sm font-bold uppercase tracking-widest text-slate-600 mb-4">{CATEGORY_LABELS[cat] || cat}</h3>
                ...
                {items.map(integ => (
                  <div key={integ.id} className="rounded-2xl p-5 flex items-start gap-4" style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${integ.enabled ? integ.color + '30' : 'rgba(255,255,255,0.08)'}` }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-slate-500 mb-3">{integ.lastSyncAt ? `Last sync: ${integ.lastSyncAt.slice(0, 16)}` : 'Never synced'}</div>
                      {integ.errorMessage && <div className="text-xs text-red-400 mb-2 truncate">{integ.errorMessage}</div>}
                      <button onClick={() => toggle(integ)}
                        className={`relative w-10 h-5 rounded-full transition-colors ${integ.enabled ? 'bg-indigo-600' : 'bg-slate-700'}`}>
