...
const MOCK_NOTIFICATIONS: Notification[] = [
  { id: '1', type: 'contact', titleEn: 'New contact request received', titleFa: 'درخواست تماس جدید دریافت شد', time: '2m ago', read: false },
  { id: '2', type: 'consultation', titleEn: 'Consultation booked for tomorrow', titleFa: 'مشاوره برای فردا رزرو شد', time: '1h ago', read: false },
  { id: '3', type: 'system', titleEn: 'Database backup completed', titleFa: 'پشتیبان‌گیری از پایگاه داده تکمیل شد', time: '3h ago', read: true },
]
...
export function AdminHeader({ user, title, locale, onToggleLocale, onOpenCmd }: Props) {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-6 border-b border-[#1e1e2e] bg-[#0c0c14]/80 backdrop-blur-sm flex-shrink-0 gap-4 relative z-40">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <button
          onClick={onOpenCmd}
          className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs text-slate-500 hover:text-slate-300 transition-all"
        ...
        <div className="relative">
          <button
            ...
            className="relative w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/5 transition-all"
          >
            🔔
            {unread > 0 && (
              <span className="absolute top-0.5 right-0.5 w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center bg-indigo-600 text-white">
                {unread}
              </span>
            )}
          </button>
            ...
            <>
              ...
              >
                  ...
                  {notifications.map(n => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-slate-300 leading-snug">{isRTL ? n.titleFa : n.titleEn}</p>
                ...
                <div className="px-4 py-2 text-center">
                  <Link href="/admin/audit" onClick={() => setNotifOpen(false)} className="text-xs text-indigo-400 hover:text-indigo-300">
        ...
        {/* Role badge */}
        <span className={`text-xs px-2 py-1 rounded-full font-medium border hidden md:inline-flex ${roleColors[user.role] || 'text-slate-400 bg-slate-400/10 border-slate-400/20'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-slate-300 hidden lg:block max-w-[100px] truncate">{user.name}</span>
        </div>
